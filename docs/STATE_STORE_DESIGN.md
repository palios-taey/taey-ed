...
**Status: v3 (2026-07-10)** — DDL hardened to CLASS-level enforcement after HORIZON NO-GO. Formal DDL `spark/state_schema.sql` (STRICT + WITHOUT ROWID + recursive_triggers=ON + class-closing triggers). Standing gate: `spark/tools/state_closure_suite.py` (25 violation-CLASS probes — Horizon A/B/C/D + Gaia set — all abort; runs in CI). **Status v1 line retained below for history.**

> **Enforcement is TWO layers (Horizon):** (1) DDL closes STRUCTURE/TYPE/SHAPE by construction — STRICT tables enforce types (kills text/second-scale ms), WITHOUT ROWID+NOT NULL kills NULL-PK identities, recursive_triggers=ON + no-clobber INSERT guards neutralize INSERT OR REPLACE, triggers close the coordination/archive/dedup classes. (2) AUTHORIZATION + evidence + who-did-this live in the app repository chokepoint `spark/state_repo.py` (p2): every state transition (clear/fold/promote/demote/archive/resolve/terminal) goes through a narrow method that verifies caller role + evidence and writes the event in the SAME transaction; operator paths never expose generic SQL. Triggers constrain shape; they cannot verify a caller — so they do not pretend to. Requires sqlite ≥ 3.42 (verified in the deploy venv: 3.45.1).
...
- `coordination` — ONE row per (platform, screen_id) escalation gate, modeling the FULL /tmp semantics enumerated by grok *(answers Claim 4, not a naive mapping)*:
  ...
  - `last_attempt_key` (climb dedup, R8.4), `resume_at` + `response_pending_until`, `notify_cycles` (knowledge gate notify exactly-once by transaction), `tier_dispatches` (per-{screen,tier,ladder-cycle} dedup, R8.12), `yaml_sha_at_attempt` (fold provenance, R8.5),
  - Artifact BLOBS (tree/screenshot always-overwrite dumps, packets, user_instructions) STAY AS FILES referenced by path — the DB owns TRANSITIONS, the filesystem owns bytes (inventory Part 4 recommendation). Operator file-pokes get CLI/API replacements (terminal/user-instructions, consult respond).
